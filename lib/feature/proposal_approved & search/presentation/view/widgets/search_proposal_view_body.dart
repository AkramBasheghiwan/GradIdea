...
class SearchProposalViewBody extends StatefulWidget {
  const SearchProposalViewBody({super.key});

  @override
  State<SearchProposalViewBody> createState() => _SearchProposalViewBodyState();
}

class _SearchProposalViewBodyState extends State<SearchProposalViewBody> {
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _textController = TextEditingController();
  late ProposalSearchBloc _bloc;

  @override
  void initState() {
    super.initState();
    _bloc = context.read<ProposalSearchBloc>();
  ...
  void _onScroll() {
    ...
    if (currentScroll >= (maxScroll * 0.9)) {
      if (_bloc.state is ProposalSearchLoaded) {
        _bloc.add(FetchNextProposalPage());
      }
    }
  }
  ...
  Widget _buildActiveFiltersRow() {
    return BlocBuilder<ProposalSearchBloc, ProposalSearchState>(
      builder: (context, state) {
        if (state is! ProposalSearchLoaded) return const SizedBox();
  ...
  Widget _buildResultsList() {
    return BlocConsumer<ProposalSearchBloc, ProposalSearchState>(
      listenWhen: (prev, current) =>
          current is ProposalSearchLoaded && current.paginationError != null,
      listener: (context, state) {
        if (state is ProposalSearchLoaded && state.paginationError != null) {
      ...
      builder: (context, state) {
        ...
        if (state is ProposalSearchError) {
          return CustomBuildProjectErrorCard(
            message: state.message,
            onTape: () => _bloc.add(SearchTextChanged(_textController.text)),
          );
        }
  ...
  void _showFilterBottomSheet() {
    ...
    if (currentState is ProposalSearchLoaded) {
